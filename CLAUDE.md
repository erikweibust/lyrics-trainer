...
A framework-free, build-step-free line trainer for memorizing Sonnet 18. Four ES modules loaded directly by `index.html` via `<script type="module">`; Vite is only a dev server, not a bundler in the shipped path. Serving over http is the supported path, and `file://` is not: a file page has a null origin, module scripts are fetched under CORS, so Chrome and Safari refuse `main.js` and render a blank card. Only Firefox still permits file-to-file module loads. Collapsing the four modules into one classic script would buy `file://` back everywhere, at the cost of the split below — not worth it.
